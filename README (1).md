...
> Runs **100% locally** using Hugging Face models. No API keys. No cloud.
...
│   ├── llm.py             ← HuggingFace local LLM (no API key)
│   └── requirements.txt
├── frontend/
│   ├── src/
...
> You will see `[llm] Model ready on cpu` when it's done.
...
Once you upload a CSV/Excel file, try asking:
...
Models are auto-downloaded on first run from Hugging Face.
...
The app will automatically use your GPU.
...
git init
git add .
git commit -m "Initial commit - IoT LLM Tool"
git branch -M main
git remote add origin https://github.com/yehaa2004/IOT_dataset_LLM.git
...
> **GitHub password prompt?** Use a Personal Access Token instead:
> Go to `https://github.com/settings/tokens` → Generate new token (classic) → tick `repo` → copy and paste it as the password.
...
| LLM | Hugging Face Transformers (local) |
...
- Model weights are excluded from Git (downloaded automatically).
